...
/*   Updated: 2021/12/27 16:28:56 by amarie-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_ranked(char **list)
{
	...
	while (list[j] != NULL)
	{
		...
		buff = rank;
		list_bis[j] = (char *)ft_calloc(ft_strlen(ft_itoa(buff)), sizeof(char));
		ft_memcpy(list_bis[j], ft_itoa(buff), ft_strlen(ft_itoa(buff)));
		rank = 0;
		j++;
	}
	ft_free(list, list_size(list));
	return (list_bis);
}
